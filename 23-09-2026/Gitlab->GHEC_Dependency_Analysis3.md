...
**Report Version:** 1.0  
**Integration:** This analysis informs the Git Lift migration pipeline (Connection → Discovery → Estimation → Planning → Migration → Validation phases).
...
- **6 Critical Blocking Decisions** required before Git Lift can proceed beyond Discovery phase
...
The analysis identifies which hierarchy decisions unblock each Git Lift migration stage and validates acceptance criteria for each decision gate.
...
| **Git Lift Stage** | Connection → Cannot establish target connection without enterprise selection |
...
| **Blocking Status** | **BLOCKING - ALL STAGES** |
| **Affected Git Lift Stages** | Connection (auth validation), Discovery (scope filtering), Estimation (capacity planning), Planning (scheduling), Migration (target setup), Validation (acceptance) |
| **Git Lift State Store Impact** | Enterprise decision required to initialize state database schema and target repository tracking |
...
**Details:**
- Single GitLab instance confirmed in scope; Git Lift discovery phase has validated API connectivity
- No multi-instance consolidation complexity identified; single instance simplifies state store initialization
- GHEC or GHEC EMU target enterprise must be pre-selected before Git Lift Connection stage can authenticate to GitHub API
- Enterprise decision cascades to all downstream organizational hierarchy decisions
- Git Lift state store requires enterprise selection to properly scope repository tracking and delta sync operations
- EMU (Enterprise Managed Users) vs. standard GHEC affects GitHub App token validation and user mapping strategy
...
| **Git Lift Organization Mapping** | 1:1 baseline: burner-group1 → "burner-group1" GitHub organization |
| **Target Organization Name** | *NEEDS VALIDATION* (proposed: "burner-group1") |
| **Repository Count After Flattening** | 50 peer repositories under single organization |
...
| **Git Lift State Store Impact** | Organization decision required to initialize repository tracking tables and migration state |
| **Dependency Status** | **Needs Validation** |
| **Blocking Status** | **BLOCKING** |
| **Affected Git Lift Stages** | Discovery (scope filtering), Estimation (complexity/capacity), Planning (team allocation), Migration (org creation), Validation (placement verification) |
| **Decision Owner** | Product Owner / Operations Leadership |

**Analysis:**
- 43 direct projects at burner-group1 root level will be created as peer repositories in target organization
- Contains 2 nested subgroups (subgroup1 parent; sub-subgroup1, sub-subgroup2 as children) with 7 contained projects
- Git Lift's flattening strategy will create all 7 nested projects as direct repositories (removing hierarchy)
- No regulatory/operational split explicitly identified in discovery data
- Flat project structure at root level enables efficient Git Lift migration (no prior flattening needed)
- **Recommendation:** Map to single target organization (50 peer repositories) unless split criteria identified
- **Migration Risk:** 50 peer repositories in one organization; Git Lift team-based grouping recommended for organizational context

**Pending Decision:**
- Confirm one-to-one organization mapping for burner-group1
- Identify any regulatory/operational boundaries requiring split
- Validate naming convention for target organization (proposed: "burner-group1")
- Define team-based grouping structure for 50 flattened repositories (subgroup context preservation)
...
**Impact:** When Git Lift flattens repository hierarchy to single organization, both cannot coexist with same name in GHEC/GHEC EMU. Git Lift's state store will attempt to create duplicate repositories and fail.

**Git Lift Integration:** This collision will cause repository creation workflow to fail during Migration stage. State database will reject duplicate repository names within same organization scope.

**Resolution Required:** BEFORE Planning stage approval; affects Git Lift repository tracking schema
...
**Impact:** Git Lift **cannot proceed past Connection stage** without target enterprise. Connection stage requires GitHub API access to validate target enterprise capacity and permissions.

**Git Lift Integration:** Enterprise selection is required input to Git Lift CLI Connection phase. State store initialization depends on enterprise context to properly scope database schema and repository tracking.

**Resolution Required:** BEFORE Connection stage execution
...
**Status:** **BLOCKING ALL STAGES** - Unblocks Connection → cascades to all downstream stages
...
**Impact:** Git Lift's **Estimation and Planning stages blocked** until organizational structure is finalized. State store requires organization context to initialize migration tracking, delta sync scope, and validation rules.

**Git Lift Integration:** Organization decisions feed into Git Lift repository assignment logic. Team-based grouping for flattened subgroups depends on organization finalization.
...
**Status:** **BLOCKING** - Prevents Estimation and Planning stages
...
**Git Lift Consideration:** Git Lift supports LFS file migration with dedicated handling. Large repository size (681.5 MB) is near GHEC recommendation threshold (5 GB) but within limits. LFS files require special handling during mirror clone and push operations.

**Impact:** LFS file migration during flattening requires validation; may affect migration duration and require monitoring

**Recommended Actions:**
1. Validate Git Lift LFS handling for this repository during dry-run (Git Lift --dry-run mode)
2. Confirm LFS pointer files maintain valid references post-migration
3. Include in Git Lift performance testing and capacity planning
4. Monitor Git Lift delta sync operations for LFS-related issues
5. Document LFS file count and size in migration tracking record

**Status:** Non-blocking; recommend inclusion in Git Lift testing and monitoring plan
...
- **Git Lift Stage Impact:** **UNBLOCKS Connection** → enables all downstream stages
- **Git Lift Integration:** Enterprise selection required to initialize state store schema and GitHub API connection validation
...
- **Git Lift Stage Impact:** **UNBLOCKS Discovery/Estimation** → downstream Planning depends on this
- **Git Lift Integration:** Organization decision required to configure repository tracking scope in state store
- **Owner:** Product Owner
...
- **Git Lift Stage Impact:** **UNBLOCKS Estimation/Planning** for organizational scope finalization
- **Git Lift Integration:** Organization count affects state store schema and repository assignment logic
- **Owner:** Product Owner
- **Timeline:** Required before Estimation stage
...
- **Git Lift Stage Impact:** **BLOCKS Planning/Migration** → state store cannot create duplicate repos
- **Git Lift Integration:** Naming collision will cause repository creation workflow failure; must resolve before Git Lift repository creation
...
- **Git Lift Stage Impact:** **UNBLOCKS Planning** → confirms flattening strategy is valid
- **Git Lift Integration:** Flattening approval required to configure Git Lift repository creation strategy; no alternative in GHEC/GHEC EMU
- **Owner:** Product Owner
...
- **Git Lift Stage Impact:** **INFORMS Planning** → team structure affects access management configuration
- **Git Lift Integration:** Team structure decision feeds into Git Lift access control configuration; manual team creation required post-migration (Git Lift creates repos, not teams)
...
The hierarchy decisions documented in this analysis directly enable the PACE Git Lift migration platform to execute the enterprise migration workflow:

#### Stage 1: Connection (Requires DECISION 1)
- **Dependency:** Enterprise account selection (DECISION 1)
- **Git Lift Action:** `pace-git-lift-cli connection --target-enterprise <enterprise-name>`
- **State Store:** Initializes connection tracking; validates GitHub App permissions
- **Blocking Gate:** Cannot proceed without enterprise selection approved

#### Stage 2: Discovery (Requires DECISIONS 1-3)
- **Dependency:** Enterprise selection + organization mapping (DECISIONS 1, 2, 3)
- **Git Lift Action:** `pace-git-lift-cli discovery --source gitlab.com --repos <scope>`
- **State Store:** Records repository metadata, hierarchy, and source context from GitLab API
- **Blocking Gate:** Organization scope must be finalized for discovery filtering

#### Stage 3: Estimation (Requires DECISIONS 1-5)
- **Dependency:** All organizational and structural decisions (DECISIONS 1-5)
- **Git Lift Action:** Complexity analysis, LFS detection (Alert 4), naming validation
- **State Store:** Complexity scores, risk assessments, resource requirements per repository
- **Blocking Gate:** Subgroup flattening approval (DECISION 5) must be confirmed

#### Stage 4: Planning (Requires ALL DECISIONS 1-6)
- **Dependency:** All decisions approved (DECISIONS 1-6)
- **Git Lift Action:** `pace-git-lift-cli plan --output migration-plan.json`
- **State Store:** Migration schedule, sequence order, team assignments, validation rules
- **Blocking Gate:** Naming collision resolution (DECISION 4) must be approved before repository creation plan

#### Stage 5: Migration (Executes planned work)
- **Dependency:** Planning approved; all hierarchy decisions confirmed
- **Git Lift Action:** `pace-git-lift-cli migrate --plan migration-plan.json --dry-run` (recommended first), then `--execute`
- **State Store:** Real-time tracking of repository creation, ref migration, metadata sync
- **Special Handling:**
  - Flattening strategy (DECISION 5): Removes all nested subgroup paths; creates peer repositories
  - Team assignment (DECISION 6): Transfers team ownership post-repository-creation
  - LFS handling (Alert 4): Validates LFS files for "big-one" repository
  - Naming collision (DECISION 4): Prevents duplicate repository creation

#### Stage 6: Validation (Post-migration verification)
- **Dependency:** Migration complete; all repositories created in target
- **Git Lift Action:** `pace-git-lift-cli validate --target-org <org-name>`
- **State Store:** Verification records, placement confirmation, team membership validation
- **Verification Points:** Confirms no naming duplicates, all 50-51 repositories placed correctly, team context preserved
...
source:
  instance: gitlab.com
  scope: [burner-group1, chiraguniworld-group]
...
target:
  ...
  organizations:
    - name: burner-group1
      repos: 50
      flattening: enabled
      subgroups: [subgroup1, sub-subgroup1, sub-subgroup2]
    - name: chiraguniworld-group (PENDING DECISION 3)
      repos: 1
      flattening: false

special_handling:
  naming_collision:
    source: [burner-group1/thisisrepo, burner-group1/subgroup1/sub-subgroup2/thisisrepo]
    resolution: <DECISION 4 - TO BE APPROVED>
  
  lfs_repositories:
    - big-one: {size: 681.5MB, lfs_files: 2}
  
  team_grouping:
    strategy: <DECISION 6 - TO BE APPROVED>
    teams:
      - subgroup1-team: [scriptRepoForDefaultBranch1]
      - sub-subgroup1-team: [sub-subgroup1project, sub-subgroup1project2, gitlift_automation_depot212, big-one]
      - sub-subgroup2-team: [scriptRepo4, <thisisrepo-renamed>]

migration_risk:
  high_volume_repos: [HighIssuesMrs]  # 1,437 MRs, 15 issues
  large_repos: [big-one]  # 681.5 MB with LFS
  medium_risk: [<additional repos from policy assessment>]

state_store:
  schema_version: 1.0
  repositories: 51
  organizations: 1-2 (depends on DECISION 3)
  status: PENDING - awaiting hierarchy decision approvals
```
...
1. **Update Git Lift Configuration**
   ...
   - Apply team grouping from DECISION 6

2. **Execute Connection Stage**
   - Git Lift connects to target enterprise
   - GitHub App credentials validated
   - State store initialized for first organization

3. **Proceed Through Remaining Stages**
   - Discovery collects source repository metadata
   - Estimation calculates complexity and timeline
   - Planning finalizes execution sequence
   - Migration executes repository creation and data sync
   - Validation confirms placement and team assignment
...
| **Integration** | This analysis is a deliverable input to the Git Lift migration platform pipeline. Hierarchy decisions enable Connection → Discovery → Estimation → Planning → Migration → Validation phases. |
| **Related Artifacts** | Git Lift Configuration (generated upon decision approval), Git Lift Migration Plan (generated during Planning stage), Git Lift State Database (tracks repository and organizational placement) |
| **Last Updated** | September 24, 2026 |
| **Next Review** | Post-decision implementation; upon Git Lift Planning stage approval |
